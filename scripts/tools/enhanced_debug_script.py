...
def analyze_video_detections(video_path: str, sample_frames: int = 10):
    ...
    try:
        ...
        for i, frame_idx in enumerate(frame_indices):
            ...
            if result.detections:
                ...
                for detection in result.detections:
                    ...
                    print(
                        f"   - {class_name}: confidence={confidence:.3f}, center=({center[0]:.1f}, {center[1]:.1f})"
                    )
                ...
                config = get_config()
                filtered_result = detector.detect(
                    rgb_frame, target_classes=config.counting.target_classes
                )

                if len(filtered_result.detections) != len(result.detections):
                    print(
                        f"   🔄 After filtering: {len(filtered_result.detections)} detections"
                    )
        ...
        for class_name, count in detection_summary.items():
            if count > 0:
                percentage = (
                    (count / total_detections) * 100 if total_detections > 0 else 0
                )
        ...
        if detection_summary["bottle-plastic"] > 0 and (
            detection_summary["bottle-glass"] == 0 and detection_summary["tin can"] == 0
        ):
            ...
            print(
                "   - Validation criteria (objects failing distance/confidence checks)"
            )
...
def test_counting_line_position():
    ...
    sample_videos = (
        list(Path(video_path).glob("*.mp4")) if Path(video_path).exists() else []
    )
        ...
        if ret:
            ...
            if line_x is not None:
                if 0 <= line_x <= width:
                    ...
                    print(
                        f"❌ Vertical line at x={line_x} is outside frame bounds (0-{width})"
                    )
            ...
            if line_y is not None:
                if 0 <= line_y <= height:
                    ...
                    print(
                        f"❌ Horizontal line at y={line_y} is outside frame bounds (0-{height})"
                    )
