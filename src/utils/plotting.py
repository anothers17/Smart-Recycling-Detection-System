...
class Colors:
    ...
    def get_class_color(
        self, class_name: str, bgr: bool = False
    ) -> Tuple[int, int, int]:
        """
...
class EnhancedAnnotator:
    ...
    def draw_label(
        ...
        try:
            ...
            (text_width, text_height), baseline = self.text_calculator.get_text_size(
                text,
                self.text_props.font_scale,
                self.text_props.thickness,
                self.text_props.font,
            )
            ...
            if self.text_props.background:
                ...
                cv2.rectangle(
                    self.image, (bg_x1, bg_y1), (bg_x2, bg_y2), background_color, -1
                )
        ...
        try:
            ...
            if background:
                ...
                (text_width, text_height), baseline = (
                    self.text_calculator.get_text_size(
    ...
    def draw_statistics(
        ...
        try:
            ...
            for i, (key, value) in enumerate(stats.items()):
                ...
                self.draw_text_at_position(
                    text, (x, current_y), (255, 255, 255), background
                )
    ...
    def draw_fps(
        self, fps: float, position: Tuple[int, int] = (10, 30)
    ) -> "EnhancedAnnotator":
        """
        Draw FPS counter.
        ...
        color = (
            (0, 255, 0) if fps >= 20 else (0, 255, 255) if fps >= 10 else (0, 0, 255)
        )
        self.draw_text_at_position(fps_text, position, color)
        return self

    def draw_confidence_bar(
        self, xyxy: List[float], confidence: float, bar_width: int = 4
    ) -> "EnhancedAnnotator":
        """
        ...
        try:
            ...
            # Background bar (gray)
            cv2.rectangle(
                self.image, (bar_x, y1), (bar_x + bar_width, y2), (128, 128, 128), -1
            )
            ...
            cv2.rectangle(
                self.image, (bar_x, conf_y), (bar_x + bar_width, y2), color, -1
            )
    ...
    def draw_center_point(
        self, xyxy: List[float], color: Tuple[int, int, int], radius: int = 3
    ) -> "EnhancedAnnotator":
        """
...
class GetTextSizeForLabel:
    ...
    def __init__(
        self, label: str, im_shape: Tuple[int, ...], line_width: Optional[int] = None
    ):
        self.label = label
...
class Annotator:
    ...
    def drawQuantity(
        self, classnames_xyxys_count_object: List[Dict[str, Any]]
    ) -> np.ndarray:
...
def create_annotator(
    image: np.ndarray, line_width: Optional[int] = None
) -> EnhancedAnnotator:
...
def draw_counting_statistics(
    image: np.ndarray, counts: Dict[str, int], position: Tuple[int, int] = (10, 30)
) -> np.ndarray:
    """
...
def create_detection_summary_image(
    detections: List[Dict[str, Any]], image_size: Tuple[int, int] = (640, 480)
) -> np.ndarray:
    """
