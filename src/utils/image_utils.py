    ...
    try:
        if len(np_array.shape) == 3:
            ...
            if ch == 3:  # RGB
                bytes_per_line = ch * w
                qt_image = QImage(
                    np_array.data, w, h, bytes_per_line, QImage.Format_RGB888
                )
            elif ch == 4:  # RGBA
                bytes_per_line = ch * w
                qt_image = QImage(
                    np_array.data, w, h, bytes_per_line, QImage.Format_RGBA8888
                )
            else:
            ...
            bytes_per_line = w
            qt_image = QImage(
                np_array.data, w, h, bytes_per_line, QImage.Format_Grayscale8
            )
        else:
...
def numpy_to_qpixmap(
    np_array: np.ndarray, width: Optional[int] = None, height: Optional[int] = None
) -> QPixmap:
    """
...
def add_text_overlay(
    ...
    try:
        ...
        # Draw text
        cv2.putText(
            result, text, (x, y), font, font_scale, color, thickness, cv2.LINE_AA
        )
...
def save_image(
    image: np.ndarray, filepath: Union[str, Path], quality: int = 95
) -> bool:
    """
    Save image to file.
...
def calculate_image_statistics(image: np.ndarray) -> dict:
    ...
    try:
        ...
        if len(image.shape) == 3:
            ...
            stats["channel_means"] = [
                float(np.mean(image[:, :, i])) for i in range(image.shape[2])
            ]
        else:
            stats["channels"] = 1
