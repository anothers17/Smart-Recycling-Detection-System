    ...
    def browse_file(self):
        ...
        try:
            ...
            filepath, _ = QFileDialog.getOpenFileName(
                self, f"Select {self.file_type} file", start_dir, file_filter
            )
...
class DetectionDisplayWidget(QWidget):
    ...
    realtimeAvailableChanged = pyqtSignal(bool)
    ...
    def setup_ui(self):
        ...
        self.realtime_radio = QRadioButton("Real-time Camera")

        self.source_button_group.addButton(self.video_radio)
        self.source_button_group.addButton(self.realtime_radio)
        self.video_radio.setChecked(True)

        source_layout.addWidget(self.video_radio)
        source_layout.addWidget(self.realtime_radio)
        file_selection_layout.addWidget(source_group)
        ...
        file_selection_layout.addWidget(self.camera_widget)
    ...
    def setup_connections(self):
        ...
        self.realtime_radio.toggled.connect(self._on_mode_changed)

        # Connect camera combo
    ...
    def setup_placeholder(self):
        ...
        cv2.putText(
            placeholder_image,
            "Select model and video file or camera to start",
            (140, 260),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (200, 200, 200),
    ...
    def set_video_file(self, filepath: str):
        ...
        self._on_source_selected(filepath)
    ...
    def _populate_cameras(self):
        ...
        if self.camera_combo.count() == 0:
            ...
            self.realtime_radio.setEnabled(False)

            self.realtime_radio.setToolTip("No cameras detected on this system")

        else:

            self.realtime_radio.setEnabled(True)
    ...
    def _on_mode_changed(self):
        ...
        if is_realtime and self.camera_combo.count() == 0:

            self.video_radio.setChecked(True)

            return

        self.video_selector.setVisible(not is_realtime)

        self.camera_widget.setVisible(is_realtime)

        if is_realtime:

            self._on_source_selected(self.camera_combo.currentData())

        else:

            current_file = self.video_selector.get_file()

            if current_file:

                self._on_source_selected(current_file)
    ...
    def _on_source_selected(self, source):
        ...
        if isinstance(source, int):
            ...
            self.videoSelected.emit(f"camera:{source}")

        else:

            self.videoSelected.emit(source)
    ...
    def set_camera(self, camera_index: int):
        ...
        self._on_source_selected(camera_index)
...
class DetectionInfoPanel(QFrame):
    ...
    def update_detection_info(
        self, detections: int, processing_time: float, class_counts: Dict[str, int]
    ):
        """
        ...
        try:
            ...
            self.add_stat(
                "Speed",
                f"{1/processing_time:.1f} FPS" if processing_time > 0 else "N/A",
            )
