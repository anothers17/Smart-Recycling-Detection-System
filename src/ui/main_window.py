...
class MainWindow(QMainWindow):
    ...
    def setup_menu_bar(self):
        ...
        for theme_name in theme_manager.get_available_themes():
            ...
            theme_action.triggered.connect(
                lambda checked, t=theme_name: self._change_theme(t)
            )
        ...
        try:
            ...
            if success:
                self.control_panel.add_log_entry(
                    f"Model loaded: {Path(model_path).name}", "SUCCESS"
                )
                ...
                self.control_panel.add_log_entry(
                    f"Failed to load model: {Path(model_path).name}", "ERROR"
                )
                self._show_error(
                    "Model Loading Error", f"Failed to load model from {model_path}"
                )
    ...
    def _on_video_selected(self, video_path: str):
        ...
        try:
            ...
            self.control_panel.add_log_entry(
                f"Video selected: {Path(video_path).name}", "SUCCESS"
            )
    ...
    def _start_processing(self):
        ...
        try:
            ...
            if self.current_video_path is None or self.current_video_path == -1:
                self._show_error(
                    "Detection Error", "Please select a valid video source first"
                )
                return
            ...
            self.video_processor.performanceUpdated.connect(
    ...
    def _on_statistics_updated(self, stats):
        ...
        try:
            ...
            if total > 0 and total % 10 == 0:  # Log every 10 detections
                self.control_panel.add_log_entry(
                    f"Total count reached: {total}", "INFO"
                )
    ...
    def _save_settings(self):
        ...
        try:
            ...
            if filepath:
                ...
                self.control_panel.add_log_entry(
                    f"Settings saved: {filepath}", "SUCCESS"
                )
    ...
    def _load_settings(self):
        ...
        try:
            ...
            if filepath:
                ...
                self.control_panel.add_log_entry(
                    f"Settings loaded: {filepath}", "SUCCESS"
                )
