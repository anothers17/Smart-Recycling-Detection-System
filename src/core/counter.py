    ...
    def update(self, detections: List[Detection]) -> Dict[str, TrackedObject]:
        ...
        for detection in unmatched_detections:
            ...
            new_object = TrackedObject(
                object_id=obj_id, class_name=detection.class_name
            )
...
class RecyclingCounter:
    ...
    def update(self, detection_result: DetectionResult) -> Dict[str, int]:
        ...
        try:
            ...
            if detection_result.detections:
                detected_classes = [
                    det.class_name for det in detection_result.detections
                ]
                logger.debug(
                    f"Frame {self.frame_count}: Detected classes: {detected_classes}"
                )
            ...
            if filtered_detections:
                logger.debug(
                    f"After filtering: {[det.class_name for det in filtered_detections]}"
                )
            ...
            for obj_id, direction in new_crossings.items():
                ...
                if obj_id not in self.crossed_objects:
                    ...
                    msg = f"✅ OBJECT COUNTED: {tracked_obj.class_name} ({direction}) - Total: {self.total_count}"
                    logger.info(msg)
    ...
    def _check_crossings(
        ...
        for obj_id, tracked_obj in tracked_objects.items():
            ...
            # Check for crossing
            crossing_direction = self.counting_line.has_crossed_line(
            ...
            if crossing_direction:
                ...
                if self._validate_crossing(tracked_obj):
                    ...
                    logger.info(
                        f"🎯 CROSSING DETECTED: {obj_id} ({crossing_direction})"
                    )
                else:
    ...
    def _validate_crossing(self, tracked_obj: TrackedObject) -> bool:
        ...
        min_distance = self.config.counting.min_distance  # Minimum pixels for valid crossing
        ...
        if avg_confidence < 0.3:
            logger.debug(f"Confidence too low: {avg_confidence:.3f} < 0.3")
            return False
    ...
    def _log_statistics(self):
        ...
        msg = f"📊 COUNTING STATS - Total: {self.total_count}, Frame: {self.frame_count}, FPS: {fps:.2f}"
        logger.info(msg)
