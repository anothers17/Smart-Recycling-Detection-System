    ...
    def create_main_window(self) -> bool:
        ...
        try:
            ...
            self._show_critical_error(
                "Startup Error", f"Failed to create main window: {e}"
            )
            return False
...
Examples:
  ...
  python src/main.py --debug                  # Enable debug logging
    ...
    parser.add_argument(
        "--model", "-m", type=str, help="Path to model file (.pt, .pth)"
    )

    parser.add_argument(
        "--video", "-v", type=str, help="Path to video file or camera index"
    )
...
def check_dependencies() -> bool:
    ...
    """
    required_modules = ["cv2", "numpy", "torch", "ultralytics", "PyQt5"]
    ...
    try:
        ...
        try:
            ...
            if parsed_args.model:
                app_manager.main_window.detection_display.set_model_file(
                    parsed_args.model
                )

            if parsed_args.video:
                app_manager.main_window.detection_display.set_video_file(
                    parsed_args.video
                )
...
def run_batch_mode(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not args.model:
            logger.error("Model file required for batch mode")
            return 1

        if not args.video:
            logger.error("Video file required for batch mode")
            return 1
        ...
        results = process_video_file(
            ...
            output_path=(
                args.video.replace(".mp4", "_processed.mp4")
...
def create_desktop_shortcut():
    ...
    try:
        if sys.platform == "win32":
            ...
            logger = get_logger("main")
            logger.info("Desktop shortcut creation not supported on this platform")

    except ImportError:
        logger = get_logger("main")
        logger.warning("Desktop shortcut creation requires: pip install winshell pywin32")
    except Exception as e:
        logger = get_logger("main")
