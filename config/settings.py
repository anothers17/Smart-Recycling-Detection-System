...
try:
    from src.version import __version__, __author__, __description__, __app_name__
except ImportError:
    # Fallback values if version file not found
    __version__ = "1.0.0"
    __author__ = "Sulhee Sama-alee"
    __description__ = "AI-powered recycling detection system using YOLOv8"
    __app_name__ = "Smart Recycling Detection"
...
class CountingConfig:
    ...
    target_classes: List[str] = field(
        default_factory=lambda: ["bottle-glass", "bottle-plastic", "tin can"]
    )
...
class AppConfig:
        ...
        for env_var, (section, attr, type_func) in env_mappings.items():
            ...
            if value is not None:
                try:
                    ...
                    import logging
                    logging.warning(f"Warning: Invalid value for {env_var}: {value}. Error: {e}")
    ...
    def _validate_config(self):
        ...
        if self.ui.theme not in valid_themes:
            import logging
            logging.warning(f"Warning: Invalid theme '{self.ui.theme}'. Using 'modern'.")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result["paths"] = {
            k: str(v) for k, v in self.paths.__dict__.items() if not k.startswith("_")
        }
